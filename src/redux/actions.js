...
export const loginUser = async (credentials, dispatch) => {
  ...
  try {
    ...
    dispatch(
      loginFailure(error.response?.data?.message || "La connexion a échoué")
    );
  }
};
...
export const updateUser = async (token, body, onCloseForm, dispatch) => {
  ...
  try {
    ...
    dispatch(
      updateFailed(
        error.response?.data?.message || "La mise à jour du profile a échoué"
      )
    );
  }
};
...
export const signUpUser = (user) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      signUpFailed(
        error.response?.data?.message || "La création du compte a échoué"
      )
    );
  }
};
